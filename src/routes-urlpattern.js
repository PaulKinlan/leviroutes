...
// Import URLPattern polyfill for environments without native support
import { URLPattern as URLPatternPolyfill } from 'urlpattern-polyfill';

// Use native URLPattern if available, otherwise use polyfill
const URLPatternImpl = typeof URLPattern !== 'undefined' ? URLPattern : URLPatternPolyfill;
...
const routes = function() {
  ...
  this.parseRoute = function(path) {
    if (URLPatternImpl) {
      ...
      try {
        ...
        console.warn('Failed to create URLPattern, falling back to regex:', e);
  ...
  var matchRoute = function(url, e) {
    ...
    for(var i = 0; route = _routes[i]; i ++) {
      ...
      // Check if this route uses URLPattern or regex
      if (route.regex.isURLPattern) {
        // Use URLPattern matching
        const result = route.regex.urlPattern.exec({ pathname: url });
        if (result) {
          matched = true;
